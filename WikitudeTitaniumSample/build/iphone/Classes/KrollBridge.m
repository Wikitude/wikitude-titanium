...
NSString * WikitudeTestApp$ModuleRequireFormat = @"(function(exports){"
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_wikitudetestapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create WikitudeTestApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "WikitudeTestApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_wikitudetestapp = [[WikitudeTestAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_wikitudetestapp];
	
	NSString *_wikitudetestappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _wikitudetestappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_wikitudetestapp valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:WikitudeTestApp$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.wikitudetestapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.wikitudetestapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
